$(document).ready(function(){
    ...
    websocket.onmessage = function(ev) {
		...
		if(!usuario){
			...
			createMessage2 (mensaje,usuario,direccion,id,tabla,accion);
		}
	};
	function createMessage (mensaje,usuario,direccion,id,tabla,accion) {
		if (tabla=="Escuela") {
			var dataString = 'valor='+id;
	        $.ajax({type: "POST",url: "http://"+window.location.hostname+"/ixtapaluca"+"/php/bsc.php?t=EscuelaSuper",data: dataString,
	            success: function(data) {
	            	//alert(data);
	              if (data==0) {
	              	if ($("#"+"Escuela"+id).length) {
						($("#"+"Escuela"+id)).addClass('label-danger');
						($("#"+"Escuela"+id)).fadeOut(1000);
					}
	              } else {
	...
	function createMessage2 (mensaje,usuario,direccion,id,tabla,accion) {
		//alert(tabla+id);
	...
	function crearTdTabla(id,tabla,tdtabla,accion){
		...
		if ($("#Escuela"+id).length) {
			if (accion=="eli") {
				...
				($("#Escuela"+id)).addClass('label-warning');
				$("#Escuela"+id).html(tdtabla);
			}
		}else{
			$('<tr id="'+"Escuela"+id+'"> '+tdtabla+' </tr>').prependTo('#resultado'+"Escuela");
			($("#"+"Escuela"+id)).addClass('label-primary');
		}
	}
